...
module.exports = function(controller, handler) {
    ...
    controller.hears(parens, CONFIG.HEAR_TYPES, function(bot, message) {
        try {
            ...
            const re = /(\B~)?(([1-9][0-9]*)?d([1-9][0-9]*)(?:([HL])([1-9][0-9]*)?)?([+-][0-9]+(?:\.[0-9]+)?)?(?:(\*|\/|\||\\|\/\/|\\\\)([0-9]+(?:\.[0-9]+)?))?)\b/ig;
            const fun = function(match, avg, slug, count, size, hilo, keep, mod, muldev, fact) {
                let expand = [];
                ...
                let text = atoms.join(' ');
                if (attach.length < CONFIG.MAX_ATTACH-1)
                    attach.push({
                        'text': text,
                        'mrkdwn_in': ['text'],
                        'color': color
